...
export class IvaoClient extends TypedEmitter<IIvaoClientEvents> {
   ...
   * ```
   */
  public getIvaoVoiceServersFiltered(options?: IGetServersFilteredOptions): IServer[] | undefined {
        ...
        if (query.keys) {
          ...
          for (let key in this.ivaoData?.clients) {
            if (keys.includes(key as GetClientsFilteredOptionsKeys)) {
              // @ts-ignore
              clients[key as keyof IClients] = this.ivaoData.clients[key as keyof IClients];

              if (query.limit) {
                // @ts-ignore
                clients[key as keyof IClients] = clients[key as keyof IClients]?.slice(
                  0,
                  query.limit
                );
              }
    ...
    if (filter.lastTrack) {
      filteredPilots = filteredPilots.filter((pilot) => {
        ...
        for (let prop in filter.lastTrack) {
          if (lastTrack[prop as keyof ILastTrack] !== filter.lastTrack[prop as keyof ILastTrack]) {
            return false;
          }
        }
        return true;
    ...
    if (filter.flightPlan) {
      ...
      filteredPilots = filteredPilots.filter((pilot) => {
        ...
        for (let prop in filter.flightPlan) {
          ...
          if (
            flightPlan[prop as keyof IFlightPlan] !== filter.flightPlan[prop as keyof IFlightPlan]
          ) {
            return false;
          }
        }
    ...
    if (filter.atcSession) {
      filteredAtcs = filteredAtcs.filter((atc) => {
        ...
        for (let prop in filter.atcSession) {
          if (
            atcSession[prop as keyof IAtcSession] !== filter.atcSession[prop as keyof IAtcSession]
          ) {
            return false;
          }
        }
   ...
   * ```
   */
  public getIvaoObserversFiltered(options?: IGetObserversFilteredOptions): IObserver[] | undefined {
